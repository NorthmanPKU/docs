...
description: "Run OpenAI Agents SDK code with Motus tracing and deployment. Change one import — keep everything else."
---

Run your existing OpenAI Agents SDK code through Motus with automatic tracing and cloud deployment. Import from `motus.openai_agents` instead of `agents` — your agent definitions, tool functions, and run logic stay exactly the same.
...
You get the following automatically — no code changes required.
...
Serve your agent with a single command:

```bash
motus serve start myapp:agent --port 8000
```

Where `agent` is an OpenAI `Agent` instance — Motus auto-detects it. No adapter import is needed.

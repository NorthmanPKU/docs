...
The processor converts each completed ADK span into Motus `task_meta` format — extracting model name, token usage, tool arguments and responses, and error types from ADK's semantic convention attributes — and ingests them into `TraceManager`. Traces are auto-exported on process exit.
